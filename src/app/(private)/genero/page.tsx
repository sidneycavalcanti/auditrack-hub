...
import { useCadSexos, useDeleteSexo } from "./hooks/useCadSexo";
import SexoFormDialog from "./components/SexoFormDialog";

export default function CadGeneroPage() {
    const [searchTerm, setSearchTerm] = useState("");
    const [statusFilter, setStatusFilter] = useState<string>("all");
    const [isDialogOpen, setIsDialogOpen] = useState(false);
    const [editingCadSexo, setEditingCadSexo] = useState<Sexo | undefined>();
    const [currentPage, setCurrentPage] = useState(1);

    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const [sexoToDelete, setSexoToDelete] = useState<Sexo | null>(null);
    const [page, setPage] = useState(1);
    const [limit] = useState(10);

    const { data: cadSexoResp, isLoading, error } = useCadSexos({
        name: searchTerm || undefined,
        page: page,
        limit: limit,
    });

    const deleteMutation = useDeleteSexo();

    const cadsexos = cadSexoResp?.data ?? [];
    const pagination = {
        total: cadSexoResp?.total ?? 0,
        totalPages: cadSexoResp?.totalPages ?? 1,
        currentPage: cadSexoResp?.page ?? 1,
        limit: cadSexoResp?.limit ?? 10,
    };

    const filteredMotivosDePausa = useMemo(() => {
        if (statusFilter === "all") return cadsexos;
        return cadsexos.filter((item) =>
            statusFilter === "active" ? item.situacao : !item.situacao
        );
    }, [cadsexos, statusFilter]);

    const handleCreate = () => {
        setEditingCadSexo(undefined);
        setIsDialogOpen(true);
    };

    const handleEdit = (cadSexo: Sexo) => {
        setEditingCadSexo(cadSexo);
        setIsDialogOpen(true);
    };

    const handleDeleteClick = (cadSexo: Sexo) => {
        setSexoToDelete(cadSexo);
        setDeleteDialogOpen(true);
    };

    const handleDeleteConfirm = async () => {
        if (sexoToDelete) {
            await deleteMutation.mutateAsync(sexoToDelete.id);
            setDeleteDialogOpen(false);
            setSexoToDelete(null);
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="flex h-full items-center justify-center">
                <LoadingSpinner size="lg" text="Carregando gêneros..." />
            </div>
        );
    }
                ...
                <CardContent className="px-6 py-0">
                    <div className="flex flex-col gap-2 sm:flex-row">
                        <div className="relative flex-1">
                            ...
                            <Input
                                placeholder="Buscar gênero cadastrado..."
                                value={searchTerm}
            ...
            {filteredMotivosDePausa.length === 0 && !isLoading ? (
                ...
                <div className="rounded-md border">
                    <Table>
                        ...
                        <TableBody>
                            {cadsexos.map((cadsexo: Sexo) => {
                                return (
                                    <TableRow key={cadsexo.id}>
                                        <TableCell className="font-medium py-1.5">{cadsexo.name}</TableCell>
                                        <TableCell className="py-1.5">
                                            <Badge
                                                variant={cadsexo.situacao ? "default" : "secondary"}
                                                className={
                                                    cadsexo.situacao
                                            ...
                                            >
                                                {cadsexo.situacao ? "Ativo" : "Inativo"}
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="py-1.5">
                                            {cadsexo.createdAt ? formatDate(cadsexo.createdAt) : "-"}
                                        </TableCell>
                                        <TableCell className="text-right py-1.5">
                                            <div className="flex items-center justify-end gap-2">
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => handleEdit(cadsexo)}
                                                    className="cursor-pointer"
                                                    aria-label={`Editar ${cadsexo.name}`}
                                                    title="Editar"
                                                >
                                                ...
                                                <Button
                                                    variant="outline"
                                                    size="sm"
                                                    onClick={() => handleDeleteClick(cadsexo)}
                                                    aria-label={`Excluir ${cadsexo.name}`}
                                                    title="Excluir"
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            {sexoToDelete?.name}"? Esta ação não pode ser desfeita.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
