...
export default function RelatorioAuditoria({ data }: Props) {
  ...
  const pizzaGenero = (g: { feminino: number; masculino: number }) => [
    { name: "Feminino", value: g.feminino, fill: "#EC4899" },
    { name: "Masculino", value: g.masculino, fill: "#2563EB" },
  ];

  const generoBars = [
    {
      nome: "Feminino",
      valor: data.perfilFrequentador.feminino,
      fill: "#EC4899",
    },
    {
      nome: "Masculino",
      valor: data.perfilFrequentador.masculino,
      fill: "#2563EB",
    },
  ];
  const totalClientes = data.clientesCompraramVsNao.total || 0;
  const pctCompraram = totalClientes
    ? (data.clientesCompraramVsNao.compraram / totalClientes) * 100
    : 0;
    ...
    <Card className="bg-transparent">
      ...
      <div className="aud-topbar print:aud-topbar-print">
        ...
        <div className="aud-topbar__title">
          <div className="aud-topbar__muc">
            {meta.muc} – {meta.loja}
          </div>
          <div className="aud-topbar__periodo">
            PERÍODO REALIZADO: {meta.mes.toUpperCase()} / {meta.ano}
          </div>
        </div>
      </div>
      ...
      <CardHeader className="print:hidden">
        ...
        <CardDescription>
          {meta.loja} • MUC: {meta.muc} • Ref.: {meta.mes}/{meta.ano}
        </CardDescription>
      </CardHeader>
          ...
          <Card className="chart-box">
            ...
            <CardContent className="h-[220px]">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie
                    data={pizzaGenero(data.perfilFrequentador)}
                    dataKey="value"
                    innerRadius={52}
                    outerRadius={84}
                  >
                    {pizzaGenero(data.perfilFrequentador).map((entry, i) => (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart
                  data={[
                    {
                      nome: "Manhã",
                      valor: data.perfilFrequentador.turnos.manha,
                    },
                    {
                      nome: "Tarde",
                      valor: data.perfilFrequentador.turnos.tarde,
                    },
                    {
                      nome: "Noite",
                      valor: data.perfilFrequentador.turnos.noite,
                    },
                  ]}
            ...
            <CardContent className="h-[260px]">
              ...
              <div className="text-center text-sm text-muted-foreground">
                ...
                <span className="font-semibold">
                  {(data.conversaoGeral.aproveitamento * 100).toFixed(1)}%
                </span>
              </div>
            </CardContent>
          </Card>
              ...
              <PieChart>
                <Pie
                  data={[
                    {
                      name: "Compraram",
                      value: data.clientesCompraramVsNao.compraram,
                    },
                    {
                      name: "Não compraram",
                      value: data.clientesCompraramVsNao.naoCompraram,
                    },
                  ]}
                  innerRadius={50}
                  outerRadius={80}
