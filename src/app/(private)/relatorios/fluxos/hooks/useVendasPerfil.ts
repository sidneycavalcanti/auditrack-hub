// FILE: src/app/(private)/relatorios/fluxos/hooks/useVendasPerfil.ts
"use client";
...
export type PerfilFilters = FilterOptions & { lojaId?: number; mes?: number; ano?: number };

const QUERY_KEY = "vendas-perfil";

const PT_WEEK = ["Segunda-feira", "Terça-feira", "Quarta-feira", "Quinta-feira", "Sexta-feira", "Sábado", "Domingo"];
...
type Row = { dia: string; masculino: number; feminino: number; crianca: number; jovem: number; adulto: number; idoso: number; total: number };

type Totals = { masculino: number; feminino: number; crianca: number; jovem: number; adulto: number; idoso: number; total: number };

type Percent = { masculino: number; feminino: number; crianca: number; jovem: number; adulto: number; idoso: number };
...
export function useVendasPerfil(filters: PerfilFilters = {}, opts?: { enabled?: boolean }) {
    ...
    return useQuery<PerfilVendasResponse>({
        ...
        queryFn: async () => {
            ...
            const list: any[] = Array.isArray(payload?.vendas) ? payload.vendas : Array.isArray(payload) ? payload : [];
            ...
            for (const v of vendas) {
                ...
                else if (fx.startsWith("adolesc")) idade.jovem[w] += 1;
            ...
            const chartGenero = PT_WEEK.map((name, i) => ({ name, Masculino: genero.masculino[i], Feminino: genero.feminino[i] }));
            const chartIdade = PT_WEEK.map((name, i) => ({ name, Criança: idade.crianca[i], Jovem: idade.jovem[i], Adulto: idade.adulto[i], Idoso: idade.idoso[i] }));
            ...
            return { data: vendas, total: totalCount, page, limit, totalPages, rows, totals, percent, chartGenero, chartIdade };
        },
    });
}
